...
say_hello
# Above we can say that the say_hello method has its own scope where the variable message is defined. This means we are free to reference and print that variable in that same scope. The say_hello method is a local scope. This will be true for any method.
...
say_hello
# Above we suffer the same issue. The say_hello scope does not have access to the surrounding scope. From our previous example it seems that a scope is determined by a method. Each method will have its own local scope. This is going to be our rule of thumb. Technically we consider code out in the open as another local scope, although it's not a method. It's common for new ruby programmers to think that variables defined out in the open will automatically be accessible in the global scope, but this is not true.
